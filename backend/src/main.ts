...
async function bootstrap() {
  ...
  const allowedHeaders = configService.get<AppConfig['CORS_ALLOWED_HEADERS']>('CORS_ALLOWED_HEADERS')
  const credentials = configService.get<AppConfig['CORS_CREDENTIALS']>('CORS_CREDENTIALS')
  ...
  const webhookPath = configService.get<AppConfig['BOT_WEBHOOK_PATH']>('BOT_WEBHOOK_PATH')
  if (webhookPath) {
